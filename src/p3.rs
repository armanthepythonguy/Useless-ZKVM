...
impl<AB: AirBuilder> Air<AB> for VMAir{
    ...
    fn eval(&self, builder: &mut AB) {
        ...
        builder.when_transition().assert_zero(next[9]*(local[1]*next[0]+next[10]-local[0]));
...
pub fn get_trace<F: Field>(vm: VM) -> RowMajorMatrix<F>{
    ...
    let imi_trace = [final_trace[final_trace.len()-11],final_trace[final_trace.len()-10],final_trace[final_trace.len()-9],final_trace[final_trace.len()-8]];
